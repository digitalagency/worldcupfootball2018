<div class="circle-line">
  <div class="container">
    <div class="row">
      <div class="col-md-4 col-sm-4">
        <div class="wrap">
          <img src="<?php echo base_url();?>content_home/images/con-cir1.png" alt="">
          <div class="low">
            <a href="http://www.youtube.com/watch?v=Do41VVl6O9M?rel=0" rel="prettyPhoto" title="How to play the contest" target="_blank">How to play the contest</a>
            <br>
            <button type="button" class="btn btn-default one" onclick="window.location='<?php echo base_url().'match-day-contest';?>'">Click to Play</button>
          </div>
        </div>
      </div>
      <div class="col-md-4 col-sm-4">
        <div class="wrap">
          <img src="<?php echo base_url();?>content_home/images/con-cir2.png" alt="">
          <div class="low">
            <a href="#"><!-- How to play the contest -->This contest will open soon.</a><br>
            <button type="button" class="btn btn-default two" onclick="window.location='<?php echo base_url().'ultimate-contest-question';?>'">Click to Play</button> 
            <!-- <button type="button" class="btn btn-default two" title="Stay Tuned!!">Click to Play</button>-->
           </div>
        </div>
      </div>
      <div class="col-md-4 col-sm-4">
        <div class="wrap">
          <img src="<?php echo base_url();?>content_home/images/con-cir3.png" alt="">
          <div class="low">
            <a href="https://www.youtube.com/watch?v=uZ8VW79HtNE" title="How to play the contest" target="_blank">How to play the contest</a><br>
            <button type="button" class="btn btn-default three" onclick="window.location='<?php echo base_url().'all-time-contest';?>'" rel="prettyPhoto" target="_blank">Click to Play</button>
          </div>
        </div>
      </div>
    </div>
  </div>
</div>